...
>(
  ...
  async ({ projectId, startDate, endDate }, { rejectWithValue }) => {
    try {
      ...
      const message = error.response?.data?.message || error.message || "리포트 데이터 요청 실패";
...
>(
  ...
  async (params, { rejectWithValue }) => {
    ...
    try {
      ...
      const filename = `${projectIdentifier}_Report_${periodSuffix}.pdf`.replace(/:/g, '-').replace(/\s+/g, '_');
      ...
      let errorMessage = "PDF 생성 또는 다운로드 중 알 수 없는 오류가 발생했습니다.";
      if (error.response && error.response.data) {
        if (error.response.data instanceof Blob) {
          try {
            errorMessage = await error.response.data.text();
          } catch (e) { /* Blob 텍스트 변환 실패 */ }
...
const reportSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchReportDetail.fulfilled, (state, action: PayloadAction<Report, string, { arg: FetchReportDetailParams }>) => {
      ...
      .addCase(fetchReportDetail.rejected, (state, action) => {
        ...
        state.error = action.payload || action.error.message || "리포트 요청 실패";
      })
      ...
      .addCase(downloadPdfReport.fulfilled, (state, action) => {
        ...
        console.log(action.payload.message, "다운로드 파일명:", action.payload.filename);
        // 필요시 여기에 성공 알림(toast 등) 로직 추가
      })
      .addCase(downloadPdfReport.rejected, (state, action) => {
        state.isGeneratingPdf = false;
        state.pdfError = action.payload || action.error.message || "PDF 생성/다운로드 실패";
        console.error("PDF 다운로드 Thunk 오류:", action.error, "페이로드:", action.payload);
      });
  },
});
